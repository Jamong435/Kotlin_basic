...
//2. 변수를 만들때 자료형을 사용하지 않고 var 키워드 사용함. 단, 자료형은 존재함. 자동 형변환 안됨. 즉, 정적타입언어
//3. 코틀린은 함수형 언어 : 함수를 객체처럼 생각해서 변수에 저장하고 파라미터 넘겨주는 등의 작업이 가능함. [ 객체지향언어가 아님 ]
...
fun main() {
    ...
    //명시적 형변환하는 방법 [ (Int)문법 없음 : .toXXX()로 변환 ]
    a = 3.14.toInt()
    b = 50.toDouble()
    println(a)
    println(b)
...
//    }
    ...
    //이런 특징 때문에 if문을 코틀린에서는 제어문이 아니라 if표현식 이라고 부름

    //switch문법이 없어졌음 [ 비슷한 녀석이 있음 ]
    var h: Any? = null
    ...
    //when을 특정 수식으로 제어 가능 [ 수식비료할때 는 ()없어야 함 ]
    h = 85
    when {
    ...
    //when도 if문처럼 변수에 저장 가능함 [반드시 else ]
    h = 20
    var result = when (h) {
        10 -> "Hello"
        20 -> "Nice"
...
//    }
    ...
    // i는 제어변수임 for문의 지역변수 - 저 위에 있는 a 아님
    for (a in 3..10) {
        println(a)
    }
    println()
    ...
    //배열의 특별한 점...
    //마치 ArrayList처럼 get(), set()메소드 보유
    println(arr.get(0))
    println(arr.get(1))
    println(arr.get(2))
    println()

    arr.set(1, 200)  //1번방에 200설정
    println(arr.get(0))
    println(arr.get(1))
    println(arr.get(2))
    println()
    ...
    // Boolean 부터 Double까지 xxxArrayOf() - String은 없음
...
//8.1에서 호출하는 함수
//함수정의시에는 무조건 fun 키워드 사용
fun show(){
    println("show function")
    println()
}

//8.2 파라미터 전달 : 매개변수에는 var키워드 사용금지 [변수명:자료형]
...
//** 참고 [리턴타입이 없으면 void가 아니라 Unit 타입
fun display(){
    println("display")
}
