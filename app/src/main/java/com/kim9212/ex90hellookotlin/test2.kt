...
fun main(){
    ...
    //1.1 다른 .kt 파일에 class만들기 [당연히 확장자 .kt]
    var obj2= MyKtclass()
    obj2.show()
...
class Simple4 constructor(){
    ...
    //보조생성자 추가하고 싶다면..명시적으로 주 생성자를 호출하는 코드가
    //옆에 있으면 됨
...
//보조생성자만 구현해도 주 생성자는 원래부터 존재하고 있은 것임
class Simple3{
    //init
    init {
    ...
    //보조생성자는 Overloading 이 가능함 [ 보조생성자 var키워드로 한번에 멤버변수 만들기 불가 ]
    constructor(num:Int){
...
class Simple2 constructor(var num:Int, num2:Int){//var키워드를 사용하면 그 멤버변수임 , mum2는 var이 없으므로 매개변수(지역변수)
    ...
    //멤버변수 에 매개변수값 대입을 생성자 블럭에서 안해도 됨
...
//1. 클래스 선언 [ 멤버변수(Field)를 Property[프로퍼티:속성] 라고 부름]
class MyClass{
